...
export class EmployeeAddressService {
    ...
    getEmployeeAddressListByEmployeeAsync(employee_id: number): Observable<EmployeeAddress[]> {
        return this.apiService.send<EmployeeAddress[]>("GET", `${environment.UrlConstants.EmployeeAddress.GetAddressesByEmployee}/${employee_id}`);
    }
    ...
    removeEmployeeAddressAsync(employee_address_id: number): Observable<any> {
        return this.apiService.send<any>("DELETE", `${environment.UrlConstants.EmployeeAddress.DeleteEmployeeAddress}/${employee_address_id}`)
    }
}