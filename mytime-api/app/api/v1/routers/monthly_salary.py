...
# ──────────────────────────────────────────────────────────────────────
@router.get("/fetch-all-with-employees", response_model=MonthlySalaryWithEmployeesListResponse)
...
@router.get("/fetch-with-employees/{salary_id}", response_model=MonthlySalaryWithEmployees)
...
@router.post("/create", response_model=MonthlySalaryResponse, status_code=status.HTTP_201_CREATED)
...
async def update_monthly_salary(
    salary_id: int,
    salary_data: MonthlySalaryUpdate,
    db: Session = Depends(get_db)
):
    """Update an existing monthly salary"""
    try:
        response = MonthlySalaryService.update_monthly_salary(db, salary_id, salary_data)
        return response
...
@router.post("/publish-existing/{salary_id}", response_model=MonthlySalaryPublishResponse)
